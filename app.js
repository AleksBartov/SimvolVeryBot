...
// Обработка навигации в тестах
bot.action("quiz_skip", async (ctx) => {
  await flowHandler.handleQuizSkip(ctx);
});

bot.action("quiz_prev", async (ctx) => {
  await flowHandler.handleQuizPrev(ctx);
});

bot.action("quiz_complete", async (ctx) => {
  const userId = ctx.from.id;
  const progress = flowHandler.userProgress.get(userId);
  if (progress) {
    const quiz = flowHandler.flow.sections.find(
      (s) => s.id === progress.quizId
    );
    await flowHandler.completeQuiz(ctx, userId, quiz);
  }
});
